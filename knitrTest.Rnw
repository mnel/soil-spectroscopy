...
<<wavelets-cr, echo = F, cache = T, results = hide>>=
## a function for processing wavelets
waveletFunction <- function(.spectra, .preProcess,.subRows, .resultsNames){
  ## load spectra
  spectra <- loadRename(.spectra$.fname, .spectra$.name)
  ## pre processe
  if(.preProcess$.type  == 'CR'){
    processedSpectra <- t(apply(spectra,1, cHullDeviation, rangeB = .preProcess$.range))
  }
  
  if(.preProcess$.type == 'none'){
    processedSpectra <- spectra[,.preProcess$.range]
  }
  
  if(.preProcess$.type == 'SGD1'){
    processedSpectra <- spectra[,.preProcess$.range]
  }
  ## calculate the wavelet decomposition
  waveletInterval <- apply(processedSpectra, 1,wd, bc = 'interval', filter.number = 2,
                    family = 'DaubExPhase', min.scale = 2 )
  ## extract the wavelet coefficients
  wvCoefs <- ldply(waveletInterval, function(x){unlist(llply(2:10,waveCoefs, wdObject=x))})
